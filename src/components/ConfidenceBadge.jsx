    ...
    <div className="flex items-center gap-4">
      <div className={`confidence-ring ${ringSize}`}>
        ...
        <motion.div
          ...
          initial={{ background: 'conic-gradient(rgba(8,145,178,0.12) 0deg, rgba(8,145,178,0.12) 360deg)' }}
          animate={{
            background: `conic-gradient(rgba(34,211,238,0.95) 0deg, rgba(52,211,153,0.9) ${percentage * 3.6}deg, rgba(15,23,42,0.8) ${percentage * 3.6}deg, rgba(15,23,42,0.8) 360deg)`,
          }}
          transition={{ duration: 0.8, ease: 'easeOut' }}
        />
        <div className="confidence-ring-core" />
        <div className={`relative text-center font-display font-bold ${toneClass}`}>
          <div className={valueSize}>{percentage}%</div>
          <div className="text-[10px] uppercase tracking-[0.28em] text-slate-500">confidence</div>
        </div>
      </div>

      <div className="space-y-2">
        <p className="text-sm uppercase tracking-[0.28em] text-slate-500">Audit confidence</p>
        <div className="h-2 w-36 overflow-hidden rounded-full bg-slate-900/70">
          <motion.div
