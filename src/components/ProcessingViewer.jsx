...
export function ProcessingViewer({
  ...
  const isClarificationMode = Boolean(supplementaryPanel);
  const feedListRef = useRef(null);
  const latestFeedRef = useRef(null);
  const revealTimerRef = useRef(null);
  const [visibleFeed, setVisibleFeed] = useState(() => activityFeed.slice(0, activityFeed.length ? 1 : 0));
  const stageGroups = [
        ...
        <div className="processing-core-header">
          <div className="space-y-4">
            <div className="status-pill">
              ...
              <span>{isClarificationMode ? 'Clarification required' : 'Live processing'}</span>
            </div>

            <div>
              <p className="text-xs uppercase tracking-[0.32em] text-slate-500">
                {isClarificationMode ? 'Validation paused' : 'Current stage'}
              </p>
              <h2 className="mt-3 font-display text-3xl text-slate-800">
                {currentStep || (isClarificationMode ? 'Awaiting clarification response' : 'Initializing workflow')}
              </h2>
              <p className="mt-3 text-xs uppercase tracking-[0.28em] text-teal-700">
                {currentAgent || (isClarificationMode ? 'Validation agent' : 'Preparing agent handoff')}
              </p>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
                {currentDetail ||
                  (isClarificationMode
                    ? 'Answer the clarification request below so the coding workflow can resume.'
                    : 'Waiting for the next processing signal.')}
              </p>
            </div>
          </div>
          ...
          <div className="processing-metrics-grid">
            ...
            <div className="processing-stat">
              <p className="processing-stat-label">
                {isClarificationMode ? 'Feed items' : 'Visible feed items'}
              </p>
  ...
  return (
    <motion.div layout transition={transition} className="h-full min-h-0 space-y-4">
      {isClarificationMode ? (
        <div className="processing-clarification-layout">
          <div className="processing-clarification-top-row">
            {processingSummary}
            {liveFeedPanel}
          </div>

          <motion.div layout transition={transition} className="processing-supplementary-shell">
            {supplementaryPanel}
          </motion.div>
        </div>
      ) : (
