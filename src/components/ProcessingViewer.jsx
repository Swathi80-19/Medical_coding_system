...
export function ProcessingViewer({
  ...
  useEffect(() => {
    ...
    if (feedListRef.current) {
      ...
      const activeBounds = activeFeedRef.current.getBoundingClientRect();
      const overTop = activeBounds.top < feedBounds.top;
      const overBottom = activeBounds.bottom > feedBounds.bottom;

      if (overTop || overBottom) {
        activeFeedRef.current.scrollIntoView({
          block: 'nearest',
          behavior: 'smooth',
        });
      }
            ...
            <div ref={feedListRef} className="processing-feed-list">
              {visibleFeed.map((item, index) => (
                <motion.div
                  ...
                  ref={item.status === 'active' ? activeFeedRef : null}
