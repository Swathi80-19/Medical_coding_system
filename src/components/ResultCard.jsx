      ...
      <div className="border-b border-white/10 pb-6">
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="space-y-4">
            <div className="status-pill bg-emerald-400/10 text-emerald-100">
              <CheckCircle2 className="h-4 w-4" />
              <span>Analysis completed</span>
            </div>
            <div>
              <h2 className="font-display text-3xl text-white">Validated coding result for note {result.note_id}</h2>
            ...
            <div className="panel space-y-4">
              ...
              <div className="space-y-4">
                {result.reasoning?.map((item, index) => (
                  <div key={index} className="rounded-[24px] border border-white/10 bg-slate-950/45 p-5">
                    <div className="flex items-start gap-4">
                      ...
                      <p className="flex-1 text-base leading-7 text-slate-100">{item}</p>
                    </div>
                  </div>
                ))}
              </div>
    ...
    <div className="panel space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 font-display text-2xl text-white">{items.length} matched outputs</h3>
        </div>
      </div>

      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.code} className="rounded-[24px] border border-white/10 bg-slate-950/45 p-5">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className={`code-badge ${accent === 'emerald' ? 'code-badge-emerald' : 'code-badge-cyan'}`}>{item.code}</p>
                <p className="mt-3 text-base text-slate-100">{item.description}</p>
              </div>
