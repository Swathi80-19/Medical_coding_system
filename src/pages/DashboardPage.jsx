...
const statusStyles = {
  ...
  processing: 'border-cyan-300/20 bg-cyan-400/10 text-cyan-100',
  completed: 'border-emerald-300/20 bg-emerald-400/10 text-emerald-100',
  needs_clarification: 'border-amber-300/20 bg-amber-400/10 text-amber-100',
};
...
export function DashboardPage() {
  ...
  return (
    <section className="page-shell">
      <div className="grid gap-px border-y border-white/10 bg-white/[0.06] 2xl:grid-cols-[0.92fr_1.08fr]">
        <div className="space-y-4 bg-slate-950/38 px-4 py-4 sm:px-6 lg:px-8">
          <div className="space-y-4 border-b border-white/10 pb-5">
            <div>
              ...
              <h1 className="mt-3 font-display text-4xl text-white md:text-5xl">Track note status across the full coding workflow.</h1>
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-3 font-display text-4xl text-cyan-100">{stat.value}</p>
                    </div>
        ...
        <div className="bg-slate-950/22 px-4 py-4 sm:px-6 lg:px-8">
          <div className="panel">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Recent notes</p>
                <h2 className="mt-3 font-display text-2xl text-white">Live queue</h2>
              </div>
              <p className="text-sm text-slate-400">Polling every 2 seconds while the dashboard is open.</p>
            </div>

            <div className="mt-6 space-y-4">
              {loading ? (
                [...Array(3)].map((_, index) => (
                  <div key={index} className="h-24 animate-pulse rounded-[24px] border border-white/10 bg-white/5" />
                ))
              ) : (
